...
3. **Motor (API Sağlayıcı / LLM Motoru):** İki farklı şekilde çalışabilir:
   - **LM Studio (Yerel):** Port `1234` (veya sizin seçtiğiniz portta) yerel olarak çalışan, OpenAI API standartlarını destekleyen yerel LLM sunucusu.
   - **Gemini API (Bulut):** Google'ın `generativelanguage.googleapis.com` üzerindeki OpenAI uyumlu API uç noktası. Bulut tabanlı, yüksek hızlı ve güçlü kod yeteneği sunar.

**Aksiyon Akışı:**
ByIbosCode CLI (Anthropic Formatı) ➜ `http://localhost:8082` (Proxy Çevirisi) ➜ LM Studio (`http://localhost:1234`) VEYA Gemini API (Bulut) ➜ LLM Modeli ➜ [Cevap Dönüşü] ➜ Proxy (Yeniden Çeviri) ➜ ByIbosCode CLI
