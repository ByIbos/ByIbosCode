...
const TARGET_PORT = process.env.TARGET_PORT ? parseInt(process.env.TARGET_PORT) : 1234;
const TARGET_MODEL = process.env.TARGET_MODEL || (process.env.GEMINI_API_KEY ? 'gemini-1.5-flash' : 'gemma4:e4b');
...
function callLMStudio(payload) {
  return new Promise((resolve, reject) => {
    ...
    const client = isGemini ? https : http;
    ...
    const req = client.request(options, res => {
      ...
      res.on('end', () => {
        ...
        catch { reject(new Error(`Model response parse error: ${data.slice(0, 200)}`)); }
      });
    });
...
const server = http.createServer((req, res) => {
  ...
  req.on('end', async () => {
    ...
    try {
      ...
      console.log('Raw model response:', JSON.stringify(lmResponse, null, 2));
...
server.listen(PROXY_PORT, () => {
  ...
  if (process.env.GEMINI_API_KEY) {
    console.log(`║  Backend      → Gemini API Cloud       ║`);
  } else {
    console.log(`║  Local Model  → http://localhost:${TARGET_PORT}  ║`);
  }
  console.log(`║  Model Name   → ${TARGET_MODEL.padEnd(23)} ║`);
  console.log('╠════════════════════════════════════════╣');
